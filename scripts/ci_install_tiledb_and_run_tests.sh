...
if [[ -z ${SUPERBUILD+x} || "${SUPERBUILD}" == "OFF" ]]; then
  ...
  if [[ "$AGENT_OS" == "Linux" ]]; then
    curl -L -o tiledb.tar.gz https://github.com/TileDB-Inc/TileDB/releases/download/2.6.1/tiledb-linux-x86_64-2.6.1-2f6b7f6.tar.gz \
    && sudo tar -C /usr/local -xvf tiledb.tar.gz
  elif [[ "$AGENT_OS" == "Darwin" ]]; then
    curl -L -o tiledb.tar.gz https://github.com/TileDB-Inc/TileDB/releases/download/2.6.1/tiledb-macos-x86_64-2.6.1-2f6b7f6.tar.gz \
    && sudo tar -C /usr/local -xvf tiledb.tar.gz
  else
    mkdir build_deps && cd build_deps \
    && git clone https://github.com/TileDB-Inc/TileDB.git -b 2.6.1 && cd TileDB \
