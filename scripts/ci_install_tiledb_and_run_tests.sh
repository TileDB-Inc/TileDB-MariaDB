...
git clone https://github.com/Shelnutt2/server.git -b MDEV-20767 ${MARIADB_VERSION}
...
&& mkdir builddir \
&& cd builddir \
&& cmake -DCMAKE_PREFIX_PATH=../build_deps/TileDB/dist -DPLUGIN_TOKUDB=NO -DPLUGIN_ROCKSDB=NO -DPLUGIN_MROONGA=NO -DPLUGIN_SPIDER=NO -DPLUGIN_SPHINX=NO -DPLUGIN_FEDERATED=NO -DPLUGIN_FEDERATEDX=NO -DPLUGIN_CONNECT=NO -DCMAKE_BUILD_TYPE=Debug -SWITH_DEBUG=1 .. \
&& make -j$(nproc) \
