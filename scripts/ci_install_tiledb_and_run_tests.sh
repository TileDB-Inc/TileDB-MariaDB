...
original_dir=$PWD
export MARIADB_VERSION=${MARIADB_VERSION:="mariadb-10.6.4"}
mkdir tmp
shopt -s extglob
...
if [[ -z ${SUPERBUILD+x} || "${SUPERBUILD}" == "OFF" ]]; then
  ...
  if [[ "$AGENT_OS" == "Linux" ]]; then
    curl -L -o tiledb.tar.gz https://github.com/TileDB-Inc/TileDB/releases/download/2.3.3/tiledb-linux-x86_64-2.3.3-9336d3f.tar.gz \
    && sudo tar -C /usr/local -xvf tiledb.tar.gz
  elif [[ "$AGENT_OS" == "Darwin" ]]; then
    curl -L -o tiledb.tar.gz https://github.com/TileDB-Inc/TileDB/releases/download/2.3.3/tiledb-macos-x86_64-2.3.3-9336d3f.tar.gz \
    && sudo tar -C /usr/local -xvf tiledb.tar.gz
  else
